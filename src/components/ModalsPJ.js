// Este arquivo manipula os endereços para empresa
...
export default class ModalsPJ extends Component {
    ...
    handleClose() {
        this.props.store.dispatch(DataApi.openModalPJ(false));
    }
    ...
    render() {
        let modalUser = this.props.store.getState().modalIndexPJ;
        return (
        	<Modal ref="refcategoryPJ" show={this.props.modalPJ} onHide={this.handleClose}>
                <Modal.Header closeButton>
                    <Modal.Title><strong>{decodeURIComponent(this.props.empresa[modalUser].userInfo.name)}</strong> - {decodeURIComponent(this.props.empresa[modalUser].userInfo.email)}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    ...
                    <form id="form-contato" onSubmit={this.handleSubmit}>
                        <div className="row">
                            <div className="col-xs-6 extras">
                                <p><strong>Street:</strong> <input name="name" type="text" className={'modal-address input useritem blocked ' + this.props.empresa[modalUser].id} ref={(input) => this.name = input } disabled defaultValue={decodeURIComponent(this.props.empresa[modalUser].address.streetName)} onChange={this.handleChange} /> </p>
                            </div>
                        ...
                        <div className="row top-buffer">
                            ...
                            <div className="col-xs-4 extras">
                                <button className="form" data-id={this.props.empresa[modalUser].id} onClickCapture={(e) => this.openEdit(e)}>Edit Address</button>
                                <button className="form close-edit ok" type="submit" data-id={this.props.empresa[modalUser].id} onClickCapture={(e) => this.closeEdit(e)}>Save</button>
                            </div>
                            <div className="col-xs-4 extras" />
                        </div>
                    </form>
